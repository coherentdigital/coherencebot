...
/**
 * This local implementation of Tika's PDF Parser allows this module
 * to call a local (PdfBox) PDFTextSplitter that provides font information in
 * the parsed result.
 * 
 * Since its intended use is for Heading extraction, it does not function
 * as full featured parser.
 * 
 * @author pciuffetti
 */
public class PDFParser extends org.apache.tika.parser.pdf.PDFParser {
	...
	if (modified != null) {
          metadata.set(Metadata.LAST_MODIFIED, dateFormat.format(modified.getTime()));
	} else if (created != null) {
          metadata.set(Metadata.LAST_MODIFIED, dateFormat.format(created.getTime()));
	}
