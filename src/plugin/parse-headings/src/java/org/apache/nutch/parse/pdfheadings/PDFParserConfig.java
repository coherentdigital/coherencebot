 ...
 * 
 * PDC: This is a local implementation of the same class in Tika and was copied
 * into this package because the original class was not public and hence not
 * extensible. I needed to override the PDFParserConfig.properties to arrange
 * for heading parsing and didn't want to mess with the
 * PDFParserCOnfig.properties used in regular Tika PDF parsing.
 * 
 * <p/>
...
public class PDFParserConfig implements Serializable {
  ...
  // Should the PDFParser _try_ to extract marked content/structure tags
  // (backoff to regular
  // text extraction if the given PDF doesn't have marked content)
  private boolean extractMarkedContent = false;

  // The character width-based tolerance value used to estimate where spaces in
  // text should be added
  private Float averageCharTolerance;

  // The space width-based tolerance value used to estimate where spaces in text
  // should be added
   ...
   * there is an IOException, this silently swallows the exception and goes back
   * to the default.
   *
   * @param is
   */
  ...
  private void init(InputStream is) {
    ...
    setEnableAutoSpace(getBooleanProp(props.getProperty("enableAutoSpace"),
        getEnableAutoSpace()));
    ...
    setSortByPosition(getBooleanProp(props.getProperty("sortByPosition"),
        getSortByPosition()));
    setExtractAcroFormContent(
        getBooleanProp(props.getProperty("extractAcroFormContent"),
            getExtractAcroFormContent()));
    setExtractBookmarksText(getBooleanProp(
        props.getProperty("extractBookmarksText"), getExtractBookmarksText()));
    setExtractInlineImages(getBooleanProp(
        props.getProperty("extractInlineImages"), getExtractInlineImages()));
    setExtractUniqueInlineImagesOnly(
        getBooleanProp(props.getProperty("extractUniqueInlineImagesOnly"),
            getExtractUniqueInlineImagesOnly()));
    setExtractFontNames(getBooleanProp(props.getProperty("extractFontNames"),
        getExtractFontNames()));

    setIfXFAExtractOnlyXFA(getBooleanProp(
        props.getProperty("ifXFAExtractOnlyXFA"), getIfXFAExtractOnlyXFA()));
    ...
    setOcrImageScale(
        getFloatProp(props.getProperty("ocrImageScale"), getOcrImageScale()));

    setExtractActions(
        getBooleanProp(props.getProperty("extractActions"), false));

    setExtractMarkedContent(
        getBooleanProp(props.getProperty("extractMarkedContent"), false));

    setSetKCMS(getBooleanProp(props.getProperty("setKCMS"), false));

    boolean checkExtractAccessPermission = getBooleanProp(
        props.getProperty("checkExtractAccessPermission"), false);
    boolean allowExtractionForAccessibility = getBooleanProp(
        props.getProperty("allowExtractionForAccessibility"), true);

    if (checkExtractAccessPermission == false) {
      // silently ignore the crazy configuration of checkExtractAccessPermission
      // = false,
    ...
    maxMainMemoryBytes = getLongProp(props.getProperty("maxMainMemoryBytes"),
        -1);
   ...
   * structure. If the PDF does not contain marked content, backoff to the
   * regular PDF2XHTML for text extraction. As of 1.24, this is an "alpha"
   * version.
   *
  ...
  public void configure(PDF2XHTML pdf2XHTML) {
    ...
    pdf2XHTML.setSuppressDuplicateOverlappingText(
        getSuppressDuplicateOverlappingText());
  }
   ...
   * document. If an XFA is found, try to process that, otherwise, process the
   * AcroForm.
   *
  ...
  /**
   * If false (the default), extract content from the full PDF as well as the
   * XFA form. This will likely lead to some duplicative content.
   *
  ...
  /**
   * If true, extract inline embedded OBXImages. <b>Beware:</b> some PDF
   * documents of modest size (~4MB) can contain thousands of embedded images
   * totaling &gt; 2.5 GB. Also, at least as of PDFBox 1.8.5, there can be
   * surprisingly large memory consumption and/or out of memory errors. Set to
   * <code>true</code> with caution.
   * <p/>
   ...
   * page. This might be desired for some use cases. However, to avoid
   * duplication of extracted images, set this to <code>true</code>. The default
   * is <code>true</code>.
   * <p/>
   * Note that uniqueness is determined only by the underlying PDF COSObject id,
   * not by file hash or similar equality metric. If the PDF actually contains
   * multiple copies of the same image -- all with different object ids -- then
   * all images will be extracted.
   * <p/>
   ...
   * setting of this parameter, the extractor will only pull out one copy of
   * each image per page. This parameter tries to capture uniqueness across the
   * entire document.
   *
   * @param extractUniqueInlineImagesOnly
   */
  public void setExtractUniqueInlineImagesOnly(
  ...
  /**
   * The PDFBox parser will throw an IOException if there is a problem with a
   * stream. If this is set to <code>true</code>, Tika's PDFParser will catch
   * these exceptions and try to parse the rest of the document. After the parse
   * is completed, Tika's PDFParser will throw the first caught exception.
   * 
   * @param catchIntermediateIOExceptions
   */
  public void setCatchIntermediateIOExceptions(
  ...
  /**
   * String representation of the image format used to render the page image for
   * OCR (examples: png, tiff, jpeg)
   * 
   * @return
   */
   ...
   *
   * @param ocrImageFormatName
   *          name of image format used to render page image
   */
  ...
  /**
   * Dots per inch used to render the page image for OCR. This does not apply to
   * all image formats.
   *
   * @param ocrDPI
   */
  ...
  /**
   * Image quality used to render the page image for OCR. This does not apply to
   * all image formats
   * 
   * @return
   */
  ...
  /**
   * Whether or not to extract PDActions from the file. Most Action types are
   * handled inline; javascript macros are processed as embedded documents.
   *
   * @param v
   */
   ...
   * Additional buffering is done using a temp file.
   * 
   * @return
   */
   ...
   * <p>
   * Whether to call
   * <code>System.setProperty("sun.java2d.cmm", "sun.java2d.cmm.kcms.KcmsServiceProvider")</code>.
   * KCMS is the unmaintained, legacy provider and is far faster than the newer
   * replacement. However, there are stability and security risks with using the
   * unmaintained legacy provider.
   * </p>
   * <p>
   * Note, of course, that this is <b>not</b> thread safe. If the value is
   * <code>false</code> in your first thread, and the second thread changes this
   * to <code>true</code>, the system property in the first thread will now be
   * <code>true</code>.
   * </p>
   * <p>
  ...
  public boolean equals(Object o) {
    ...
    if (getEnableAutoSpace() != config.getEnableAutoSpace())
      return false;
    if (getSuppressDuplicateOverlappingText() != config
        .getSuppressDuplicateOverlappingText())
      return false;
    if (getExtractAnnotationText() != config.getExtractAnnotationText())
      return false;
    if (getSortByPosition() != config.getSortByPosition())
      return false;
    if (getExtractAcroFormContent() != config.getExtractAcroFormContent())
      return false;
    if (getExtractBookmarksText() != config.getExtractBookmarksText())
      return false;
    if (getExtractInlineImages() != config.getExtractInlineImages())
      return false;
    if (getExtractUniqueInlineImagesOnly() != config
        .getExtractUniqueInlineImagesOnly())
      return false;
    ...
    if (getOcrDPI() != config.getOcrDPI())
      return false;
    if (getCatchIntermediateIOExceptions() != config
    ...
    if (!getOcrImageFormatName().equals(config.getOcrImageFormatName()))
      return false;
    if (getExtractActions() != config.getExtractActions())
      return false;
  ...
  public String toString() {
    return "PDFParserConfig{" + "enableAutoSpace=" + enableAutoSpace
        ...
        + ", spacingTolerance=" + spacingTolerance + ", ifXFAExtractOnlyXFA="
        + ifXFAExtractOnlyXFA + ", ocrStrategy=" + ocrStrategy + ", ocrDPI="
        + ocrDPI + ", ocrImageType=" + ocrImageType + ", ocrImageFormatName='"
