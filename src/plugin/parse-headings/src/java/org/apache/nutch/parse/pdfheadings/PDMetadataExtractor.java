...
class PDMetadataExtractor {
  ...
  static void extract(PDMetadata pdMetadata, Metadata metadata,
    ...
    extractMultilingualItems(metadata, TikaCoreProperties.DESCRIPTION, null,
        dcSchema);
    ...
    try {
      if (xmp != null) {
        ...
        XMPSchemaPDFAId pdfaxmp = (XMPSchemaPDFAId) xmp
            .getSchemaByClass(XMPSchemaPDFAId.class);
        if (pdfaxmp != null) {
        ...
        // TODO WARN if this XMP version is inconsistent with document header
        // version?
      }
    } catch (IOException e) {
      metadata.set(
          TikaCoreProperties.TIKA_META_PREFIX + "pdf:metadata-xmp-parse-failed",
          "" + e);
    }
  }
   ...
   * attributes...despite standards documentation. JempBox expects one or the
   * other for specific attributes. Until more flexibility is added to JempBox,
   * Tika will have to handle both.
   *
   * @param schema
   * @param name
   ...
   * @param schema
   */
  private static void extractMultilingualItems(Metadata metadata,
      Property property, String pdfBoxBaseline, XMPSchema schema) {
    ...
    for (String lang : schema
        .getLanguagePropertyLanguages(property.getName())) {
      String value = schema.getLanguageProperty(property.getName(), lang);

      if (value != null && value.length() > 0) {
        // if you're going to add it below in the baseline addition, don't add
        // it now
  ...
  private static void extractDublinCoreListItems(Metadata metadata,
      Property property, XMPSchemaDublinCore dc) {
  ...
  static void addMetadata(Metadata metadata, Property property,
      Calendar value) {
    if (value != null) {
